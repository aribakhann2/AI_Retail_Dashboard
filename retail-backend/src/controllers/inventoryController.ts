...
// ✅ CONNECT INVENTORY SHEET (NOW ALSO SUBSCRIBES WEBHOOK)
...
export const handleSheetWebhook = async (req: Request, res: Response) => {
  try {
    ...
    if (!inventory) {
      console.warn(`⚠️ No inventory found for channelId: ${channelId}`);
    ...
    if (!products.length || !sales.length) {
      console.warn("⚠️ Missing products or sales data.");
...
// 6️⃣ Convert map to array and sort chronologically
...
export const updateProductStock = async (req: AuthRequest, res: Response) => {
  try {
    ...
    // 5️⃣ Fetch the "products" tab data
    ...
    // 8️⃣ Create notification after successful stock update
